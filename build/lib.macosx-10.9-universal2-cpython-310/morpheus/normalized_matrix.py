...
class NormalizedMatrix(matrix):
    ...
    def __new__(cls, ent_table, att_table, kfkds,
        ...
        shape = (sum(sizes), len(kfkds[0])) if trans else (
            len(kfkds[0]), sum(sizes))
        obj = N.ndarray.__new__(NormalizedMatrix, shape=shape, dtype=np.float64)
        ...
        obj.trans = trans
        obj.stamp = time.time() if stamp is None else stamp
        obj.sizes = sizes
    ...
    def _right_matrix_multiplication(self, n_matrix, other):
        other = other.astype(np.float64, order='C')
        s = n_matrix.ent_table
        k = n_matrix.kfkds
        r = n_matrix.att_table
